...
def getSesion(url=''):
    '''
    广东依据获取ID 的url来区分数据类型
    :param url:
    :return:
    '''
    resp = requests.get(url, headers={'User-Agent': random.choice(agent_list)}, allow_redirects=False, timeout=60)
    if int(resp.status_code) == 200:
        return sorted(re.findall('sessionID=(\d*)"', resp.content.decode('gbk')), key=lambda x: len(x), reverse=True)[0]
    # TODO  引发错误
