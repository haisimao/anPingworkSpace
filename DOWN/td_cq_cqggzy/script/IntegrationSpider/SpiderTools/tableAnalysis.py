...
class htmlTableTransformer(object):
    ...
    def table_tr_td(self, e_table, fill_method=None):
        ...
        for index, row in df.iteritems():
            results[row[0]] = '|'.join(list(df[index])[1:]).replace(row[0], '')
        return results
    ...
    def tableTrTdRegulation(self, e_table, fill_method=None):
        ...
        titleList = []
        results = {}
        td_1 = e_table.tbody.findAll('tr')[0]
        for td in td_1.findAll('td'):
            titleList.append(td.getText())

        for tr in e_table.tbody.findAll('tr')[1:]:
            tds = tr.findAll('td')
            for _ in range(len(tds)):
                strTd = results.get(titleList[_]) if results.get(titleList[_]) else ''
...
if __name__ == '__main__':
    ...
    url = 'https://www.cqggzy.com/xxhz/014004/014004001/014004001001/20200421/57cd6a05-a22f-4b37-ae4c-c83a52f9769b.html'
    ...
    results = html_table.tableTrTdRegulation(table)
