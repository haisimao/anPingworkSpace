...
class LandChinaBot:
    ...
    def getCityInfo(self, city_code, city_name):
        ...
        city_info = unquote(f'b4a43cbb-3c47-47ee-81cf-8b993d5bda89:{city_code}' + u"▓~" + city_name)
            ...
            try:
                ...
                for sub_raw in info[1:]:
                    ...
                    for det in details:
                        ...
                        all_data = {
                                    '城市': '', '行政区': '', '供应公告标题': '', '公告编号': '', '公告类型': '', '发布时间': '', '网上创建时间': '',
                                    '宗地编号：': '', '宗地面积：': '', '宗地坐落：': '', '出让年限：': '', '容积率：': '', '建筑密度(%)：': '',
                                    '绿化率(%)：': '', '建筑限高(米)：': '', '土地用途：': '',
                                    '投资强度：': '', '保证金：': '', '起始价：': '', '加价幅度：': '', '挂牌开始时间：': '',
                                    '挂牌截止时间：': '',
    ...
    async def getDetail(self, link, session):
        ...
        if not info_all:
            return False
        # print(info_all)
        names = ['宗地编号：', '宗地面积：', '宗地坐落：', '出让年限：', '容积率：', '建筑密度(%)：',
                 '绿化率(%)：', '建筑限高(米)：', '土地用途：',
                 '投资强度：', '保证金：', '起始价：', '加价幅度：', '挂牌开始时间：',
                 '挂牌截止时间：',
                 '提交书面申请地', '缴纳竞买保证金截止时间', '确认竞买资格时间', '拍卖开始时间', '拍卖挂牌进行地点',
