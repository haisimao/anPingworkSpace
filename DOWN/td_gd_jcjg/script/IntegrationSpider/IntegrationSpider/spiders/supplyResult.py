...
class supplyResultSpider(CrawlSpider):
    ...
    def __new__(cls):
        if not hasattr(cls, "instance"):
            cls.instance = super(supplyResultSpider, cls).__new__(cls)
            try:
                ...
                if os.path.exists(pathDetail):
                    ...
                    cls.fileDetail = open(pathDetail, 'a+')
                    fileds = ['行政区', '土地坐落', '总面积(公顷)', '土地用途', '签订日期', '项目名称', '项目位置',
                              '面积(公顷)', '土地来源',
                              '供应方式', '土地使用年限', '行业分类', '土地级别', '成交价格(万元)', '分期支付约定-支付期号', '分期支付约定-约定支付日期',
                              '分期支付约定-约定支付金额(万元)', '分期支付约定-备注', '土地使用权人',
                              '约定容积率下限', '约定容积率上限', '约定交地时间', '约定开工时间', '约定竣工时间',
                              '实际开工时间', '实际竣工时间', '批准单位', '合同签订日期', '爬取时间', '爬取地址url', '唯一标识', '\n']
                    cls.fileDetail.write(','.join(fileds))
            except Exception as e:
                cls.log('supplyResultSpider文件打开失败,At{}'.format(datetime.datetime.now()), level=logging.ERROR)
