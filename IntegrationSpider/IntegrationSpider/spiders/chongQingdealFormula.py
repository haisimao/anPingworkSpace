...
class chongQingdealFormulaSpider(CrawlSpider):
    ...
    def __new__(cls):
        if not hasattr(cls, "instance"):
            ...
            cls.pathDetail = os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))), 'data/重庆公共资源交易_成交公告_重庆.csv')
            ...
            if os.path.exists(cls.pathDetail):
                ...
                with open(cls.pathDetail, 'a+') as fp:
                    fp.write("""文件标题,信息时间,正文标题,公告序号,宗地编号/编号,地块位置,土地用途/用途,土地面积(平方米)/土地面积(m2)/出让面积(m),容积率,建筑密度(%),绿地率(%),保证金(万元),底价(万元),计容建筑面积(m2),出让方式,出让年限,成交价(万元)/成交价,受让单位,土地使用条件,交易时间,成交人,备注,联系单位,联系地址,联系电话,公示期,爬取时间,爬取地址url,唯一标识,\n""")
        return cls.instance
    ...
    def parse_detail(self, response):
        try:
            ...
            if '土地使用条件' in items or '宗地编号' in items or '公告序号' in items:
                ...
                # 出让年限
                CRNX_43 = '|'.join(re.findall(f'出让年限(?:[\s]*)({self.reStr})(?:\s*)成交价',  items))
                # 成交价(万元) / 成交价
