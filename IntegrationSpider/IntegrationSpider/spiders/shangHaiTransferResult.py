...
class shangHaiTransferResultSpider(CrawlSpider):
    ...
    def __new__(cls):
        if not hasattr(cls, "instance"):
            ...
            try:
                cls.pathPage = os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), 'Logs/DatashangHaiTransferResultPage.txt')
                cls.pathDetail = os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))), 'data/深圳市土地房产交易中心_土地_深圳.csv')
                ...
                if not os.path.exists(cls.dirName):
                ...
                if os.path.exists(cls.pathDetail):
                    ...
                    with open(cls.pathDetail, 'a+') as fp:
                        fp.write("""交易时间,交易状态,宗地号,土地位置,起始价,土地用途,土地面积,交易方式,交易类型,宗地,发布时间,交易状态,中标人/竞得人,成交价(元),保证金 (元),起始价 (元),竞价阶梯 (元),封顶价 (元),竞买申请截止时间,竞买人数,正文标题,公告期,挂牌开始时间,挂牌结束时间,宗地代码/地块宗地编号,宗地号,土地位置,土地用途,准入行业类别,土地面积/土地面积(平方米),建筑面积(平方米)/总建筑面积,土地使用年期/土地使用年限,土地发展建设现状,容积率,挂牌起始价(人民币万元),竞买(投标)保证金(人民币万元),土地使用年限(年),保证金截止时间,地址,电话,宗地号,土地面积,建筑面积,容积率,建筑覆盖率,建筑高度,用途,使用年限,区域,位置,绿地率,建筑楼层,竞买人,竞买出价(元),竞价时间,状态,正文标题,发布日期,宗地号,竞得人,中标人,位置,土地用途,土地面积,建筑面积,起始价,成交价,溢价率,综合楼面单价,公告编号,报名时间,爬取时间,爬取地址url,唯一标识,附件,\n""")
    ...
    def parse_detail(self, response):
        try:
            ...
            # 中标人 / 竞得人
            ZBR_24 = ''.join(data.xpath('//div[@class="right_first"]/div[1]/div[2]/text()').extract()) if data.xpath('//div[@class="right_first"]/div[1]/div[2]/text()') else ''
            # 成交价(元)
            ...
            # 封顶价(元)
            FDJ_29 = ''.join(data.xpath('//div[@class="right_first twin"][2]/div[2]/div[2]/text()').extract()) if data.xpath('//div[@class="right_first twin"][2]/div[2]/div[2]/text()') else ''
            # 竞买申请截止时间
            ...
            # TODO 竞价记录 完成
            try:
                resultsRecord = requests.post('https://www.sz68.com/tiaim/web/landrecord?t=1589334927332', headers=self.header, data={'id': id}).json().get('params').get('offers')

                # 竞买人
                JMR = '|'.join([_.get('bidder_name') for _ in resultsRecord])
                # 竞买出价(元)
                JMSJ = '|'.join([str(_.get('price')) for _ in resultsRecord])
                # 竞价时间
                CJSJ = '|'.join([_.get('offer_date') for _ in resultsRecord])
                # 状态
                ZT = '|'.join([str(_.get('status')).replace('0', '无效出价').replace('1', '有效出价').replace('2', '成交价') if _.get('status') else '' for _ in resultsRecord])
            except Exception as e:
                JMR = ''
                # 竞买出价(元)
                JMSJ = ''
                ...
                # 状态
                ZT = ''
                self.log(f'详情页数据 竞价记录 解析失败, 错误: {e}', level=logging.ERROR)
            ...
            if detailData.status_code == 200:
                ...
                # 公告编号
                try:
                    GGBH_1 = reFunction(f'(深土交告[{self.reStr}]*号)根据', items)
                except:
                    GGBH_1 = ''
                # 正文标题
                ...
                for _ in range(len(list(tdData.values())[0])):
                    # 宗地代码 / 地块宗地编号
                    ZDDM_DKZDBH_ = tdData.get('宗地编号') if tdData.get('宗地编号') else tdData.get('地块宗地编号') if tdData.get('地块宗地编号') else tdData.get('宗地代码')
                    ZDDM_DKZDBH = ZDDM_DKZDBH_[_] if ZDDM_DKZDBH_ else ''
                    # 宗地号
                    ZDH_A = tdData.get('宗地号')[_] if tdData.get('宗地号') else ''
                    # 土地位置
                    DKWZ = tdData.get('土地位置')[_] if tdData.get('土地位置') else ''
                    # 土地用途
                    DKYT = tdData.get('土地用途')[_] if tdData.get('土地用途') else ''
                    # 准入行业类别
                    ZRHYLB = tdData.get('准入行业类别')[_] if tdData.get('准入行业类别') else ''
                    # 土地面积 / 土地面积(平方米)
                    TDMJ_A_ = tdData.get('土地面积（平方米）') if tdData.get('土地面积（平方米）') else tdData.get('土地面积')
                    TDMJ_A = TDMJ_A_[_] if TDMJ_A_ else ''
                    # 建筑面积(平方米) / 总建筑面积
                    JZMJ_ = tdData.get('建筑面积（平方米）') if tdData.get('建筑面积（平方米）') else tdData.get('总建筑面积')
                    JZMJ_ = JZMJ_[_] if JZMJ_ else ''
                    # 挂牌起始价(人民币万元)
                    GPQSJ = tdData.get('挂牌起始价（人民币、万元）')[_] if tdData.get('挂牌起始价（人民币、万元）') else ''
                    # 竞买(投标)保证金(人民币万元)
                    JMBZJ = tdData.get('竞买（投标）保证金（人民币、万元）')[_] if tdData.get('竞买（投标）保证金（人民币、万元）') else ''
                    # 土地使用年限(年)
                    TDSYNX = tdData.get('土地使用年期')[_] if tdData.get('土地使用年期') else ''
                    # 爬取时间
                    ...
                    csvFile = [JYSJ, JYZT, ZDH, TDWZ, QSJ, TDYT, TDMJ,
                               JYFS_A, JYLX, ZD, FBSJ, JYZT_A, ZBR_24, CJJ_25, BZJ_26, QSJ_A, JJJT_28, FDJ_29,
                               JMSQJZSJ_30,
                               JMRS_31, ZWBT, GGQ, GPKSSJ, GPJSSJ, ZDDM_DKZDBH, ZDH_A, DKWZ, DKYT, ZRHYLB, TDMJ_A, JZMJ,
                               TDSYNX, TDFZXZ, RJL, GPQSJ, JMBZJ, TDSYNX, ZBJJZSJ, DZ, DH,
                               ZDH_B, TDMJ_B, JZMJ_B, RJL_A, JZFGL, JZGD, YT, SYNX, QY, WZ, LDL, JZLC, JMR, JMSJ, CJSJ,
                               ZT,
            ...
            # md5Mark = encrypt_md5(ZDH + WZ + ZWBT + url)
            # csvFile = [JYSJ, JYZT, ZDH, TDWZ, QSJ, TDYT, TDMJ,
            #            JYFS_A, JYLX, ZD, FBSJ, JYZT_A, ZBR_24, CJJ_25, BZJ_26, QSJ_A, JJJT_28, FDJ_29, JMSQJZSJ_30,
            #            JMRS_31, ZWBT, GGQ, GPKSSJ, GPJSSJ, ZDDM_DKZDBH, ZDH_A, DKWZ, DKYT, ZRHYLB, TDMJ_A, JZMJ,
            #            TDSYNX, TDFZXZ, RJL, GPQSJ, JMBZJ, TDSYNX, ZBJJZSJ, DZ, DH,
